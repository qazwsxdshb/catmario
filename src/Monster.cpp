...
void App::Monsteract(glm::vec2 monsterpos,int value,glm::vec2 playerpos,std::vector<std::shared_ptr<Monster>> mon) {
    ...
    if (mon[value]->name=="state" || mon[value]->name=="state2") {
        ...
        else if(mon[value]->name!="fireball" && yy>0 && UDCollision(xx,yy-1,monsterpos,-1,mon[value]->acceleration[0],mon[value]->mon_hei+2,mon[value]->mon_wei)){
    ...
    else if (mon[value]->name=="flag" && Collision((int)(playerpos.x),(int)(playerpos.y),playerwidth,playerheight,(int)(monsterpos.x),(int)(monsterpos.y),mon[value]->mon_wei,mon[value]->mon_hei * mon[value]->mon_mul) && (playerpos.y>(monsterpos.y+(13 * mon[value]->mon_mul)+6))){
        printf("%f %f",playerpos.y,monsterpos.y+(13 * mon[value]->mon_mul)+6);
        printf("aaa\n");
        mon[value]->SetVisible(1);
        Acceleration=10;
        sec=25;
    ...
    // die
    if (mon[value]->name!="claude" && mon[value]->name!="boxSpiked" && mon[value]->name!="state2" && mon[value]->name!="greenbat") {
